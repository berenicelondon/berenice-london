...
export function AnalyticsDashboard() {
  ...
  const events = useMemo(() => {
    ...
    return getEvents().filter((event: any) => {
  ...
  const conversionBreakdown = useMemo(() => {
    const conversions = events.filter((e: any) => e.type === 'conversion')
    const breakdown = conversions.reduce((acc: Record<string, number>, event: any) => {
      const type = event.action
      acc[type] = (acc[type] || 0) + 1
      return acc
    }, {} as Record<string, number>)

    return Object.entries(breakdown).map(([type, count]) => ({ type, count: count as number }))
  }, [events])
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              +{formatNumber(events.filter((e: any) => e.type === 'page_view').length)} this period
            </p>
          </CardContent>
        </Card>
